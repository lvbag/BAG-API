...
Rule: wanneer een parameterwaarde niet aan de parameterspecificaties voldoet wordt een 400 fout gegeven met de reden in invalidParams
  ...
  Abstract Scenario: Ongeldige queryparameter waarde bij zoeken
    ...
    Voorbeelden:
      ...
      | panden                | ?identificatie=a                                                          | identificatie        | integer   | Waarde is geen geldige integer.                            |
      | panden                | ?coordinates=[null, 474479.898]                                           | coordinates          | number    | Waarde is geen geldige number.                             |
      ...
      | nummeraanduidingen    | ?postcode=123aa                                                           | postcode             | pattern   | Waarde voldoet niet aan patroon ^[1-9]{1}[0-9]{3}[A-Z]{2}. |
      | nummeraanduidingen    | ?geconstateerd=B                                                          | geconstateerd        | enum      | Waarde heeft geen geldige waarde uit de enumeratie.        |
      ...
      | panden                | ?point=abc,def                                                            | point                | number    | Waarde is geen geldig decimaal getal.                      |
      ...
      | panden                | ?point=98095.02                                                           | point                | minItems  | Array bevat minder dan 2 items.                            |
      | panden                | ?point=98095.02,438495.09,0                                               | point                | maxItems  | Array bevat meer dan 2 items.                              |
  ...
  Abstract scenario: meerdere parameter waarden voldoen niet aan gespecificeerde type
    ...
    Voorbeelden:
    | path              | query string                                                                                                            | parameter     | code             | reason                                              |
    | verblijfsobjecten | ?pandIdentificatie=0345100002016017X&geldigOp=23-04-2019                                                                | identificatie | integer          | Waarde voldoet niet aan patroon ^[0-9]{16}$.        |
    | adressen          | ?postcode=1234AA&huisnummer=a&huisletter=3                                                                              | huisnummer    | number           | Waarde is geen geldige number.                      |
    | panden            | ?geldigOp=23-04-2019&beschikbaarOp=23-04-2019T00:00:00                                                                  | geldigOp      | date             | Waarde is geen geldige datum.                       |
    | /panden           | ?point=98095.02,438495.09&statusPand=Pand%20niet%20in%20gebruik&geconstateerd=null&bouwjaar[min]=-1&bouwjaar[max]=10000 | statusPand    | enumeratiewaarde | Waarde heeft geen geldige waarde uit de enumeratie. |
    | /panden           | ?point=98095.02,438495.09&statusPand=Pand%20in%20gebruik&geconstateerd=null&bouwjaar[min]=-1&bouwjaar[max]=10000        | geconstateerd | enumeratiewaarde | Waarde heeft geen geldige waarde uit de enumeratie. |
  ...
  Scenario: meerdere parameter waarden buiten het gespecificeerde bereik
    ...
    | /panden  | ?point=98095.02,438495.09&statusPand=Pand%20in%20gebruik&geconstateerd=true&bouwjaar[min]=-1&bouwjaar[max]=10000 | bouwjaar   | minimum | Waarde is lager dan minimum {minimum}.         |
    | /panden  | ?point=98095.02,438495.09&statusPand=Pand%20in%20gebruik&geconstateerd=true&bouwjaar[min]=-1&bouwjaar[max]=10000 | bouwjaar   | maximum | Waarde is hoger dan maximum {maximum}.         |
...
Rule: combineren van verschillende zoekcombinaties is niet toegestaan
  ...
  Abstract Scenario: combinatie van opgegeven parameters wordt niet ondersteund
    ...
    | /panden                | ?adresseerbaarObjectIdentificatie=0484010002033603&point=98095.02,438495.09                                         |
    | /panden                | ?adresseerbaarObjectIdentificatie=0484010002033603&bbox=135228,457502,135246,457457                                 |
    | /panden                | ?nummeraanduidingIdentificatie=0484200002040489&point=98095.02,438495.09                                            |
    | /panden                | ?nummeraanduidingIdentificatie=0484200002040489&bbox=135228,457502,135246,457457                                    |
    | /panden                | ?point=98095.02,438495.09&bbox=135228,457502,135246,457457                                                          |
    | /panden                | ?point=98095.02,438495.09&statusPand=Pand%20in%20gebruik&geconstateerd=true&bouwjaar[min]=1950&bouwjaar[max]=1979   |

  Abstract Scenario: page, pageSize, fields en expand mogen gecombineerd worden met elke zoekcombinatie
    Als '<path><query string>' wordt aangeroepen
    Dan bevat de response ten minste de volgende velden
    | naam   | waarde |
    | title  | OK     |
    ...
    | /panden                | ?point=98095.02,438495.09&page=1&pageSize=20&fields=geometrie                                                                                 |
...
Rule: parameters geconstateerd, type, gebruiksdoelen, oppervlakte, statusPand, geconstateerd en bouwjaar mogen alleen in combinatie met bbox worden gebruikt
  ...
  Abstract Scenario: status, geconstateerd en bouwjaar zonder bbox
    ...
    | ?nummeraanduidingIdentificatie=0599200000193766&geconstateerd=true                |
    | ?point=98095.02,438495.09&bouwjaar[min]=1950&bouwjaar[max]=1979                   |
...
Rule: waarde van geometrie coördinaten moet overeenkomen met opgegeven CRS
  ...
  Abstract Scenario: zoekgeometrie die niet klopt met de opgegeven of default CRS geeft een foutmelding
    ...
    | /panden                | ?point=51.93201,4.55980                 | point     |
...
Rule: een gebruiker moet geautoriseerd zijn om de API te kunnen gebruiken
...
Rule: wanneer een methode wordt aangeroepen die niet wordt ondersteund dan wordt een 405 fout gegeven
  ...
  Scenario: methode niet toegestaan
    ...
    Dan bevat de response ten minste de volgende velden
    | naam   | waarde                           |
    | title  | Deze methode is niet toegestaan. |
    | status | 405                              |
...
Rule: wanneer een bron service niet beschikbaar is wordt een 503 fout gegeven
  ...
  Scenario: Bronservice is niet beschikbaar
    ...
    Dan bevat de response ten minste de volgende velden
    | naam   | waarde                           |
    | title  | Bronservice is niet beschikbaar. |
    | status | 503                              |
